...
const popupElement=document.createElement('div');popupElement.id='aqua-popup';popupElement.style.position='fixed';popupElement.style.top='200px';popupElement.style.left='50%';popupElement.style.transform='translateX(-50%)';popupElement.style.zIndex='100001';popupElement.style.cursor='move';popupElement.innerHTML=`
    ...
    `;(function makeDraggable(popup,headerSelector){let isDragging=!1,startX,startY,startLeft,startTop;const header=popup.querySelector(headerSelector)||popup;header.style.cursor='move';function onMouseDown(e){if(e.button!==0)return;isDragging=!0;popup.style.transform='';startX=e.clientX;startY=e.clientY;const rect=popup.getBoundingClientRect();startLeft=rect.left;startTop=rect.top;document.addEventListener('mousemove',onMouseMove);document.addEventListener('mouseup',onMouseUp);e.preventDefault()}
